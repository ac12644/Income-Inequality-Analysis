---
title: "Measuring inequality: Lorenz curves and Gini coefficients"
...
## <span style="color: #0C9875;"> Part 5.1 Measuring income inequality</span>
...
<p>We will start by using income decile data from the Global Consumption and Income Project to draw Lorenz curves and compare changes in the income distribution of a country over time. Note that income here refers to market income, which does not take into account taxes or government transfers (see Section 5.10 of Economy, Society, and Public Policy for further details).</p>
...
The data is now imported (like a spreadsheet for R). Let’s use the 'head' and 'tail' function to look at the first and last few rows:  
...
<h4 style="color: #0C9875;">5.2 Calculating cumulative shares using the `cumsum` function  </h4>
...
<h4 style="color: #0C9875;">5.3 Drawing Lorenz curves</h4>
...
As the chart shows, the income distribution has changed more clearly for China (from the blue to the green line) than for the US (from the orange to the red line).   
 ...
 <h4 style="color: #0C9875;">5.4 Calculating Gini coefficients</h4>
...
<h4 style="color: #0C9875;">5.5 Calculating Gini coefficients for all countries and all years using a loop</h4>
...
<h4 style="color: #0C9875;">5.6 Plotting time series of Gini coefficients, using ggplot</h4>
...
<br>
## <span style="color: #0C9875;"> Part 5.2 Measuring other kinds of inequality</span>
 <br>
 <br>

 <h4 style="color: #0C9875;">5.7 Importing `.csv` files into R</h4>
...
```
 ...
 <h4 style="color: #0C9875;">5.8 Creating line graphs with `ggplot`</h4>
...
Most of the code below is similar to our use of `ggplot` in previous walk-throughs, though this time we added the option `labs` to change the vertical axis label (`y = `) and the option `scale_color_brewer` to change the colour palette (t clearly differentiate the lines for each country). 
...
<h4 style="color: #0C9875;">5.9 Drawing a column chart with sorted values</h4>

#### Ploting a column chart for 1952
...
#### Ploting a column chart for 2002
...
<h4 style="color: #0C9875;">5.10 Drawing a column chart with sorted values</h4>
...
<h4 style="color: #0C9875;">5.11 Using line and bar charts to illustrate changes in time</h4>

#### Importing data and plot a line chart

First we import the data into R and check its structure.

```{r}
# Open the csv file from the working directory
data_prim <- read.csv("gender-gap-in-primary-education.csv")
str(data_prim)
```
...
#### Ploting a line chart for a selection of countries
...
#### Ploting a column chart with sorted values
